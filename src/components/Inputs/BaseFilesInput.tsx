...
import { Images, CloudUpload, X, Video, FileText } from "lucide-react";
...
function StatusBadge({ color, label }: { color: "blue" | "green" | "red"; label: string }) {
  const cls = { blue: "bg-blue-100 text-blue-600 dark:bg-blue-950/40", green: "bg-green-100 text-green-700 dark:bg-green-950/40", red: "bg-red-500 text-white" }[color];
...
export function BaseFilesInput({ name, label, multiple = false, accept = "image/*,video/*,.pdf,.doc,.docx,.xlsx,.xls", attachment = false, model, value, onChange, onLoadingChange, error, touched = false }: BaseFilesInputProps) {
  ...
  return (
    <div className="space-y-2">
      {label && <div className="flex items-center gap-2"><div className={`flex size-7 items-center justify-center rounded-full ${hasError ? "bg-red-500/10 text-red-500" : "bg-purple-500/10 text-purple-600"}`}><Images size={14} /></div><span className={`text-sm font-semibold ${hasError ? "text-red-500" : "text-text"}`}>{label}</span></div>}
      <div className={`overflow-hidden rounded-2xl border-2 transition-all duration-200 ${hasError ? "border-red-400 bg-red-50/30 dark:bg-red-950/10" : "border-slate-300 dark:border-slate-600 bg-white/80 dark:bg-slate-800/40"}`}>
        {isEmpty && (
          <label htmlFor={`${name}_input`} onDragOver={(e) => { e.preventDefault(); setActiveDrag(true); }} onDragLeave={() => setActiveDrag(false)} onDrop={(e) => { e.preventDefault(); setActiveDrag(false); const f = Array.from(e.dataTransfer?.files ?? []); if (f.length) handleFiles(f); }} className={`flex cursor-pointer flex-col items-center justify-center gap-4 py-12 transition-all duration-300 ${activeDrag ? "scale-95 opacity-50" : ""}`}>
            <div className={`flex size-20 items-center justify-center rounded-full border-2 border-dashed transition-colors ${hasError ? "border-red-400 text-red-400" : "border-slate-300 dark:border-slate-600 text-app-muted"}`}><CloudUpload size={32} /></div>
            <p className="text-sm font-medium text-text">{t("BUTTONS.uploadFile")}</p>
          </label>
        )}
        {!isEmpty && (
          <div className={`${multiple ? "grid grid-cols-2 gap-3 p-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5" : "p-0"}`}>
            {files.map((f) => (
              <div key={f.id} className={multiple ? "relative flex flex-col items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 p-2" : "relative"}>
                {f.type === "image" ? <ImagePreviewTrigger src={f.src} alt={f.file?.name ?? "image"} className={multiple ? "h-[100px] w-[100px] rounded-lg object-cover" : "h-[200px] w-full rounded-t-2xl object-cover"} wrapperClassName={multiple ? "rounded-lg" : "rounded-t-2xl w-full"} />
                  : <div onClick={() => setPreview(f)} className={`flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg bg-linear-to-br from-purple-600 to-blue-500 text-white uppercase text-xs ${multiple ? "h-[100px] w-[100px]" : "h-[200px] w-full rounded-t-2xl"}`}>{f.type === "video" ? <Video size={30} /> : <FileText size={30} />}<span>{f.ext || f.type}</span></div>}
                {multiple && f.file?.name && <p className="max-w-[120px] truncate text-xs text-app-muted" title={f.file.name}>{f.file.name}</p>}
                {f.isUploading && f.percentage > 0 && <div className={`${multiple ? "w-full" : "absolute bottom-0 left-0 right-0 px-3 pb-2"}`}><div className="h-2 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700"><div className="flex h-2 items-center justify-center rounded-full bg-purple-600 transition-all" style={{ width: `${f.percentage}%` }} /></div><p className="mt-0.5 text-center text-[10px] text-app-muted">{f.percentage}%</p></div>}
                ...
                <button type="button" disabled={deletingId === f.id} onClick={() => setConfirmFile(f)} className={`flex items-center justify-center rounded-full bg-red-500 text-white shadow-md transition-all hover:bg-red-600 disabled:opacity-50 ${multiple ? "h-7 w-7" : "absolute -top-2 inset-e-2 size-7"}`}><X size={14} /></button>
              </div>
            ))}
            {multiple && (
              <label htmlFor={`${name}_input`} onDragOver={(e) => { e.preventDefault(); setActiveDrag(true); }} onDragLeave={() => setActiveDrag(false)} onDrop={(e) => { e.preventDefault(); setActiveDrag(false); const f = Array.from(e.dataTransfer?.files ?? []); if (f.length) handleFiles(f); }} className={`flex min-h-[140px] cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-600 p-2 transition-all ${activeDrag ? "scale-95 opacity-50 border-purple-400" : "hover:border-purple-400"}`}>
                <div className="flex items-center justify-center rounded-full border-2 border-dashed border-slate-300 dark:border-slate-600 p-2 text-app-muted"><CloudUpload size={24} /></div>
                <p className="text-xs text-app-muted text-center">{t("BUTTONS.addMore")}</p>
              </label>
            )}
          </div>
        )}
      ...
      {hasError && <p className="flex items-center gap-1.5 text-xs text-red-500"><span className="inline-block h-1 w-1 rounded-full bg-red-500 shrink-0" />{error}</p>}
      {confirmFile && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setConfirmFile(null)} /><div className="relative z-10 w-full max-w-sm overflow-hidden rounded-2xl bg-white dark:bg-slate-900 shadow-2xl"><div className="p-6"><h3 className="text-base font-bold text-text">{t("MESSAGES.deleteFileTitle")}</h3><p className="mt-1 text-sm text-app-muted">{t("MESSAGES.deleteFileBody")}</p></div><div className="flex justify-end gap-3 border-t border-slate-100 dark:border-slate-800 px-6 py-4"><button type="button" onClick={() => setConfirmFile(null)} className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-2 text-sm font-semibold text-app-muted hover:border-slate-300 transition-all">{t("BUTTONS.cancel")}</button><button type="button" onClick={confirmDelete} className="rounded-xl bg-red-500 px-4 py-2 text-sm font-semibold text-white hover:bg-red-600 transition-all">{t("BUTTONS.delete")}</button></div></div></div>
      )}
      {preview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setPreview(null)} /><div className="relative z-10 w-full max-w-3xl overflow-hidden rounded-2xl bg-white dark:bg-slate-900 shadow-2xl"><div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800"><h3 className="text-sm font-bold text-text">{t("MESSAGES.filePreview")}</h3><button type="button" onClick={() => setPreview(null)} className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 text-app-muted hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"><X size={14} /></button></div><div className="p-4">{preview.type === "video" && <video src={preview.src} controls className="h-[500px] w-full rounded-xl object-contain bg-black" />}{preview.type === "file" && preview.ext === "pdf" && <iframe src={preview.src} className="h-[500px] w-full rounded-xl" />}{preview.type === "file" && ["docx","doc","xlsx","xls"].includes(preview.ext) && <iframe src={`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(preview.src)}`} className="h-[500px] w-full rounded-xl" allowFullScreen />}</div></div></div>
      )}
    </div>
  );
}
