...
function StatusBadge({ color, label }: { color: "blue" | "green" | "red"; label: string }) {
  const cls = { blue: "bg-blue-100 text-blue-600 dark:bg-blue-950/40", green: "bg-green-100 text-green-700 dark:bg-green-950/40", red: "bg-red-500 text-text" }[color];
...
export function BaseFilesInput({ name, label, multiple = false, accept = "image/*,video/*,.pdf,.doc,.docx,.xlsx,.xls", attachment = false, model, value, onChange, onLoadingChange, error, touched = false }: BaseFilesInputProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={`overflow-hidden rounded-2xl border-2 transition-all duration-200 ${hasError ? "border-destructive bg-destructive/10 dark:bg-destructive/20" : "border-border bg-panel"}`}>
        {isEmpty && (
          <label htmlFor={`${name}_input`} onDragOver={(e) => { e.preventDefault(); setActiveDrag(true); }} onDragLeave={() => setActiveDrag(false)} onDrop={(e) => { e.preventDefault(); setActiveDrag(false); const f = Array.from(e.dataTransfer?.files ?? []); if (f.length) handleFiles(f); }} className={`flex cursor-pointer flex-col items-center justify-center gap-4 py-12 transition-all duration-300 ${activeDrag ? "scale-95 opacity-50" : ""}`}>
            <div className={`flex size-20 items-center justify-center rounded-full border-2 border-dashed transition-colors ${hasError ? "border-destructive text-destructive" : "border-border text-muted"}`}><CloudUpload size={32} /></div>
            ...
            {files.map((f) => (
              <div key={f.id} className={multiple ? "relative flex flex-col items-center gap-2 rounded-xl border border-border p-2" : "relative"}>
                {f.type === "image" ? <ImagePreviewTrigger src={f.src} alt={f.file?.name ?? "image"} className={multiple ? "h-[100px] w-[100px] rounded-lg object-cover" : "h-[200px] w-full rounded-t-2xl object-cover"} wrapperClassName={multiple ? "rounded-lg" : "rounded-t-2xl w-full"} />
                  : <div onClick={() => setPreview(f)} className={`flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg bg-primary text-text uppercase text-xs ${multiple ? "h-[100px] w-[100px]" : "h-[200px] w-full rounded-t-2xl"}`}>{f.type === "video" ? <Video size={30} /> : <FileText size={30} />}<span>{f.ext || f.type}</span></div>}
                {multiple && f.file?.name && <p className="max-w-[120px] truncate text-xs text-muted" title={f.file.name}>{f.file.name}</p>}
                {f.isUploading && f.percentage > 0 && <div className={`${multiple ? "w-full" : "absolute bottom-0 left-0 right-0 px-3 pb-2"}`}><div className="h-2 w-full overflow-hidden rounded-full  "><div className="flex h-2 items-center justify-center rounded-full bg-primary transition-all" style={{ width: `${f.percentage}%` }} /></div><p className="mt-0.5 text-center text-[10px] text-muted">{f.percentage}%</p></div>}
                ...
                <button type="button" disabled={deletingId === f.id} onClick={() => setConfirmFile(f)} className={`flex items-center justify-center rounded-full bg-red-500 text-text shadow-md transition-all hover:bg-red-600 disabled:opacity-50 ${multiple ? "h-7 w-7" : "absolute -top-2 inset-e-2 size-7"}`}><X size={14} /></button>
              </div>
            ))}
            {multiple && (
