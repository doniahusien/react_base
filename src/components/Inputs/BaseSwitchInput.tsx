...
export function BaseSwitchInput({ name, label, value = false, onChange, disabled = false }: BaseSwitchInputProps) {
  const uid = useId();
  const id = `${uid}-${name}`;
  return (
    <div className="flex items-center gap-3">
      <label id={id} className={`heart-switch${value ? " is-active" : ""}${disabled ? " opacity-50 pointer-events-none" : ""}`} aria-pressed={value} onClick={() => !disabled && onChange?.(!value)}>
