...
const schema = { name_ar: "required:name_ar|type:string", name_en: "required:name_en|type:string", image: "required:image|type:object" };
      ...
      <div className="relative -mx-6 overflow-hidden bg-linear-to-r from-[#0f0a2a]/10 via-[#1a0f45]/6 to-[#0a1628]/8 dark:from-[#0f0a2a] dark:via-[#1a0f45] dark:to-[#0a1628] px-6 py-7 border-b border-purple-500/10 dark:border-purple-500/20 mb-8">
        <div className="relative">
          ...
          <div className="flex items-end gap-4">
            ...
            <h1 className="text-2xl font-black tracking-tight text-slate-800 dark:text-white">{editing ? t("TITLES.edit", { count: t("TITLES.category") as any }) : t("TITLES.add", { count: t("TITLES.category") as any })}</h1>
          </div>
        </div>
      </div>
      <Form schema={schema} values={{ ...values, image: imageFile ?? imageValue }} onSubmit={handleSubmit}>
        {({ errors, field, touch }) => (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-end gap-3 pt-2 pb-4">
              <button type="button" onClick={() => navigate("/categories")} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50 px-5 py-2.5 text-sm font-semibold text-app-muted hover:text-text transition-all"><X size={15} />{t("BUTTONS.cancel")}</button>
