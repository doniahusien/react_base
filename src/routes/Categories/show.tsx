...
export default function CategoryShow() {
  ...
  if (!category) return <div className="space-y-3 p-6"><p className="font-semibold text-text">{t("TITLES.notFound")}</p><a href="/categories" className="text-sm text-purple-600 hover:underline">← {t("TITLES.categories")}</a></div>;
        ...
        <div className="relative flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-app-muted">
              ...
              <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${category.is_active ? "bg-green-500/10 text-green-600" : "bg-slate-200 dark:bg-slate-700 text-app-muted"}`}>{category.is_active ? t("TITLES.active") : t("TITLES.inactive")}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button type="button" onClick={() => navigate(`/categories/form/${category.id}`)} className="inline-flex items-center gap-2 rounded-xl border border-purple-500/30 bg-purple-50 dark:bg-purple-950/20 px-4 py-2 text-sm font-semibold text-purple-600 hover:bg-purple-100 transition-all"><Edit size={14} />{t("TITLES.edit", { count: "" as any })}</button>
            <Deleter url={`/categories/${category.id}`} onReload={() => navigate("/categories")} />
          </div>
        </div>
      </div>
      <div className="space-y-5 pb-8">
        <div className="rounded-2xl border border-border bg-white/70 dark:bg-slate-800/40 overflow-hidden shadow-sm"><div className="px-6 py-5"><SectionHeading icon={Library} title={t("TITLES.translations")} /><div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="p-4 bg-body border border-border rounded-lg"><div className="flex items-center gap-2 mb-2"><div className="w-6 h-6 rounded-full bg-blue-500/10 flex items-center justify-center"><span className="text-xs font-bold text-blue-600">EN</span></div><span className="text-sm font-medium text-app-muted">{t("TITLES.english")}</span></div><p className="text-base font-medium text-text">{category.en?.name || "—"}</p></div>
          <div className="p-4 bg-body border border-border rounded-lg"><div className="flex items-center gap-2 mb-2"><div className="w-6 h-6 rounded-full bg-green-500/10 flex items-center justify-center"><span className="text-xs font-bold text-green-600">AR</span></div><span className="text-sm font-medium text-app-muted">{t("TITLES.arabic")}</span></div><p className="text-base font-medium text-text" dir="rtl">{category.ar?.name || "—"}</p></div>
        </div></div></div>
